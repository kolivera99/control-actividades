...
const CH = {
  dash: String.fromCharCode(8212), gear: String.fromCharCode(9881),
  check: String.fromCharCode(10003), cross: String.fromCharCode(10005),
  ...
  n: String.fromCharCode(241), N: String.fromCharCode(209),
  a: String.fromCharCode(225), A: String.fromCharCode(193),
  e: String.fromCharCode(233), i: String.fromCharCode(237),
  o: String.fromCharCode(243), q: String.fromCharCode(191),
  checkG: String.fromCharCode(9989),
  warn: String.fromCharCode(9888) + String.fromCharCode(65039),
};

const today = () => new Date().toISOString().split("T")[0];
const daysDiff = (d) => { if (!d) return null; return Math.ceil((new Date(d) - new Date(today())) / 86400000); };
const fmtDate = (d) => { if (!d) return CH.dash; const [y, m, dd] = d.split("-"); return `${dd}/${m}/${y}`; };
const getMonday = (date) => {
  const d = new Date(date || new Date());
  const day = d.getDay(); const diff = d.getDate() - day + (day === 0 ? -6 : 1);
  d.setDate(diff); d.setHours(0,0,0,0); return d;
};
const getSunday = (date) => { const m = getMonday(date); m.setDate(m.getDate() + 6); m.setHours(23,59,59,999); return m; };
...
/* SESSION */
const getSession = () => { try { const s = JSON.parse(localStorage.getItem("cc-session")); if (!s) return null; if (new Date(s.expires_at) < new Date()) { localStorage.removeItem("cc-session"); return null; } return s; } catch (e) { return null; } };
const setSession = (user) => { const expires = new Date(); expires.setHours(expires.getHours() + SESSION_HOURS); const s = { ...user, expires_at: expires.toISOString(), last_active: new Date().toISOString() }; localStorage.setItem("cc-session", JSON.stringify(s)); return s; };
const refreshSession = () => { const s = getSession(); if (s) { const expires = new Date(); expires.setHours(expires.getHours() + SESSION_HOURS); s.expires_at = expires.toISOString(); s.last_active = new Date().toISOString(); localStorage.setItem("cc-session", JSON.stringify(s)); } };
...
.nav-tabs { display:flex; gap:2px; margin-bottom:20px; background:var(--white); padding:4px; border-radius:var(--radius-lg); border:1px solid var(--gray-light); box-shadow:var(--shadow-sm); }
.nav-tab { padding:8px 20px; font-size:13px; font-weight:500; color:var(--gray-mid); background:transparent; border:none; border-radius:var(--radius); cursor:pointer; transition:all .2s; font-family:var(--font); } .nav-tab:hover { color:var(--gray-dark); } .nav-tab.active { background:var(--red); color:var(--white); box-shadow:var(--shadow-sm); }
...
.progress-bar-wrap { width:80px; background:var(--gray-light); border-radius:10px; height:8px; overflow:hidden; display:inline-block; vertical-align:middle; }
.progress-bar-fill { height:100%; border-radius:10px; transition:width .3s; }
.progress-cell { display:flex; align-items:center; gap:6px; white-space:nowrap; }
.progress-input { width:42px; padding:2px 4px; border:1.5px solid var(--gray-light); border-radius:4px; font-size:11px; font-family:var(--mono); text-align:center; outline:none; } .progress-input:focus { border-color:var(--red); }
.dash-grid { display:grid; grid-template-columns:repeat(auto-fill,minmax(280px,1fr)); gap:16px; margin-bottom:20px; }
.dash-card { background:var(--white); border:1px solid var(--gray-light); border-radius:var(--radius-lg); padding:20px; box-shadow:var(--shadow-sm); transition:all .2s; } .dash-card:hover { box-shadow:var(--shadow-md); border-color:var(--gray-mid); }
...
.dash-check { font-size:16px; }
.dash-bar-wrap { width:100%; background:var(--gray-light); border-radius:10px; height:10px; overflow:hidden; margin:10px 0; }
.dash-bar-fill { height:100%; border-radius:10px; transition:width .5s; }
.dash-stats { display:flex; gap:12px; flex-wrap:wrap; }
.dash-stat { font-size:11px; color:var(--gray-dark); } .dash-stat b { font-family:var(--mono); }
.week-nav { display:flex; align-items:center; gap:10px; margin-bottom:16px; }
.week-btn { padding:6px 14px; font-size:12px; font-weight:500; background:var(--white); border:1.5px solid var(--gray-light); border-radius:var(--radius); cursor:pointer; font-family:var(--font); color:var(--gray-dark); transition:all .2s; } .week-btn:hover { border-color:var(--gray-mid); } .week-btn.active { background:var(--red); color:var(--white); border-color:var(--red); }
.week-label { font-size:13px; color:var(--gray-dark); font-weight:500; }
@media(max-width:768px) { .main { padding:16px; } .topbar { padding:0 16px; } .stats { grid-template-columns:repeat(2,1fr); } .modal { padding:24px 20px; } .dash-grid { grid-template-columns:1fr; } }
`;

/* COMPONENTS */
const ProgressBar = ({ value, size = "sm" }) => {
  const v = Math.max(0, Math.min(100, value || 0));
  const color = v >= 100 ? "var(--green)" : v >= 50 ? "var(--blue)" : v > 0 ? "var(--yellow)" : "var(--gray-light)";
  const h = size === "lg" ? 10 : 8;
  const w = size === "lg" ? "100%" : 80;
  return (
    <div style={{ width: w, background: "var(--gray-light)", borderRadius: 10, height: h, overflow: "hidden", display: "inline-block", verticalAlign: "middle" }}>
      <div style={{ width: `${v}%`, height: "100%", borderRadius: 10, background: color, transition: "width .3s" }} />
    </div>
  );
};

const DaysBadge = ({ fecha, estado }) => {
  if (estado === "completado" || estado === "cancelado") return <span style={{ fontSize: 11, color: "var(--gray-mid)" }}>{CH.dash}</span>;
  const d = daysDiff(fecha);
  if (d === null) return null;
  const style = { fontSize: 10, fontFamily: "var(--mono)", padding: "2px 8px", borderRadius: 10, fontWeight: 700, whiteSpace: "nowrap" };
  if (d < 0) return <span style={{ ...style, background: "var(--danger-bg)", color: "var(--danger)" }}>{Math.abs(d)}d vencido</span>;
  if (d <= 7) return <span style={{ ...style, background: "var(--yellow-bg)", color: "#92400E" }}>{d}d</span>;
  return <span style={{ ...style, background: "var(--green-bg)", color: "#065F46" }}>{d}d</span>;
};
...
const Login = ({ onLogin }) => {
  ...
  return (
    <div className="login-wrap"><div className="login-card">
      <div className="logo-area"><CajaLogo size={96} /><div className="logo-text"><h1>caja <span style={{ fontWeight: 800 }}>cusco</span></h1><span>Control de Actividades</span></div></div>
      <div className="red-line" />
      <p className="login-subtitle">{"Ingresa tu contrase" + CH.n + "a para acceder al sistema."}</p>
      <div className="field"><label>{"Contrase" + CH.n + "a"}</label>
        <input className="input" type="password" placeholder={CH.dots} value={pw} onChange={(e) => { setPw(e.target.value); setErr(""); }} onKeyDown={(e) => e.key === "Enter" && submit()} autoFocus />
      </div>
      {err && <div className="err">{err}</div>}
      <button className="btn btn-red" onClick={submit} style={{ marginTop: 8 }}>{loading ? "Verificando..." : "Ingresar"}</button>
    </div></div>
  );
};

const TaskForm = ({ onClose, onSave, areasList, edit, userArea }) => {
  const [f, setF] = useState(edit ? { actividad: edit.actividad, area: edit.area, responsable: edit.responsable, fecha_inicio: edit.fecha_inicio || today(), fecha_limite: edit.fecha_limite || "", estado: edit.estado, notas: edit.notas || "", descripcion: edit.descripcion || "", prioridad: edit.prioridad || "media", avance: edit.avance || 0 } : { actividad: "", area: userArea || (areasList[0] && areasList[0].nombre) || "", responsable: "", fecha_inicio: today(), fecha_limite: "", estado: "pendiente", notas: "", descripcion: "", prioridad: "media", avance: 0 });
  const upd = (k, v) => setF((p) => ({ ...p, [k]: v }));
  const ok = f.actividad.trim() && f.area && f.responsable.trim() && f.fecha_limite;
  const [saving, setSaving] = useState(false);
  const handleSave = async () => { if (!ok || saving) return; setSaving(true); await onSave({ ...f, id: edit ? edit.id : undefined }); setSaving(false); onClose(); };
  return (
    <div className="overlay" onClick={onClose}><div className="modal" onClick={(e) => e.stopPropagation()}>
      ...
      <div className="field"><label>{CH.A + "rea"}</label><select className="input" value={f.area} onChange={(e) => upd("area", e.target.value)} disabled={!!userArea}>{(userArea ? [{ nombre: userArea }] : areasList).map((a) => <option key={a.nombre} value={a.nombre}>{a.nombre}</option>)}</select></div>
      <div className="field"><label>Actividad</label><input className="input" placeholder={"Descripci" + CH.o + "n de la actividad"} value={f.actividad} onChange={(e) => upd("actividad", e.target.value)} autoFocus /></div>
      <div className="field"><label>Responsable</label><input className="input" placeholder="Nombre" value={f.responsable} onChange={(e) => upd("responsable", e.target.value)} /></div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12 }}>
        ...
        <div className="field"><label>Avance %</label><input className="input" type="number" min="0" max="100" value={f.avance} onChange={(e) => upd("avance", Math.max(0, Math.min(100, parseInt(e.target.value) || 0)))} /></div>
      </div>
      ...
      <div className="modal-foot"><button className="btn btn-outline" onClick={onClose}>Cancelar</button><button className="btn btn-ghost" onClick={handleSave} style={{ opacity: ok ? 1 : 0.5 }}>{saving ? "Guardando..." : edit ? "Guardar" : "Crear"}</button></div>
    </div></div>
  );
};
...
const DetailModal = ({ task, onClose, onEdit }) => {
  ...
  return (
    <div className="overlay" onClick={onClose}><div className="modal" onClick={(e) => e.stopPropagation()} style={{ width: 600 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 16 }}>
        <div><span className="area-tag" style={{ marginBottom: 8, display: "inline-block" }}>{task.area}</span><h2 style={{ marginBottom: 4 }}>{task.actividad}</h2></div>
      ...
      <div style={{ background: "var(--gray-bg)", borderRadius: "var(--radius)", padding: "12px 16px", marginBottom: 16, border: "1px solid var(--gray-light)" }}>
        <div style={{ fontSize: 10, fontWeight: 700, color: "var(--gray-mid)", textTransform: "uppercase", marginBottom: 6 }}>Avance</div>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}><ProgressBar value={task.avance} size="lg" /><span style={{ fontFamily: "var(--mono)", fontSize: 14, fontWeight: 700 }}>{task.avance || 0}%</span></div>
      </div>
      {task.descripcion && (<div style={{ background: "var(--gray-bg)", borderRadius: "var(--radius)", padding: "14px 16px", marginBottom: 16, border: "1px solid var(--gray-light)" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12, marginBottom: 16 }}>
        <div style={{ background: "var(--gray-bg)", borderRadius: "var(--radius)", padding: "10px 14px", border: "1px solid var(--gray-light)" }}><div style={{ fontSize: 10, fontWeight: 700, color: "var(--gray-mid)", textTransform: "uppercase", marginBottom: 4 }}>Responsable</div><div style={{ fontSize: 13, fontWeight: 600 }}>{task.responsable}</div></div>
        <div style={{ background: "var(--gray-bg)", borderRadius: "var(--radius)", padding: "10px 14px", border: "1px solid var(--gray-light)" }}><div style={{ fontSize: 10, fontWeight: 700, color: "var(--gray-mid)", textTransform: "uppercase", marginBottom: 4 }}>Inicio</div><div style={{ fontSize: 13, fontFamily: "var(--mono)" }}>{fmtDate(task.fecha_inicio)}</div></div>
        <div style={{ background: "var(--gray-bg)", borderRadius: "var(--radius)", padding: "10px 14px", border: "1px solid var(--gray-light)" }}><div style={{ fontSize: 10, fontWeight: 700, color: "var(--gray-mid)", textTransform: "uppercase", marginBottom: 4 }}>{"L" + CH.i + "mite"}</div><div style={{ fontSize: 13, fontFamily: "var(--mono)", color: active && d < 0 ? "var(--danger)" : active && d <= 7 ? "var(--yellow)" : undefined }}>{fmtDate(task.fecha_limite)}</div></div>
      </div>
      {task.notas && (<div style={{ background: "var(--yellow-bg)", borderRadius: "var(--radius)", padding: "12px 16px", marginBottom: 16, border: "1px solid rgba(245,158,11,0.2)" }}>
        <div style={{ fontSize: 10, fontWeight: 700, color: "#92400E", textTransform: "uppercase", marginBottom: 4 }}>Notas de seguimiento</div>
        <p style={{ fontSize: 13, color: "#92400E", lineHeight: 1.6 }}>{task.notas}</p>
      </div>)}
      <div className="modal-foot"><button className="btn btn-outline" onClick={onClose}>Cerrar</button><button className="btn btn-ghost" onClick={() => { onClose(); onEdit(task); }}>{CH.pencil + " Editar"}</button></div>
    </div></div>
  );
};
...
const ConfigModal = ({ onClose, areas, onUpdate }) => {
  ...
  const add = () => { const n = na.trim(); if (!n || la.find((a) => a.nombre === n)) return; setLa((p) => [...p, { nombre: n, password: np || n.toLowerCase().replace(/[\s/]/g, "").slice(0, 12), isNew: true }]); setNa(""); setNp(""); };
  ...
  const handleSave = async () => { setSaving(true); await onUpdate(la); setSaving(false); setSaved(true); setTimeout(onClose, 600); };
  return (
    <div className="overlay" onClick={onClose}><div className="modal" onClick={(e) => e.stopPropagation()} style={{ width: 580 }}>
      ...
      <div>{la.map((a) => (<div className="area-row" key={a.nombre}><span className="aname">{a.nombre}</span><div style={{ display: "flex", alignItems: "center", gap: 8 }}><input className="input" style={{ width: 130, padding: "5px 10px", fontSize: 12, fontFamily: "var(--mono)" }} value={a.password || ""} onChange={(e) => updPw(a.nombre, e.target.value)} placeholder={"nueva contrase" + CH.n + "a"} /><button className="btn btn-danger-outline btn-sm" onClick={() => rm(a.nombre)}>{CH.cross}</button></div></div>))}</div>
      <div className="add-row"><input className="input" placeholder={"Nueva " + CH.a + "rea"} value={na} onChange={(e) => setNa(e.target.value)} onKeyDown={(e) => e.key === "Enter" && add()} /><input className="input" style={{ maxWidth: 130, fontFamily: "var(--mono)", fontSize: 12 }} placeholder={"contrase" + CH.n + "a"} value={np} onChange={(e) => setNp(e.target.value)} onKeyDown={(e) => e.key === "Enter" && add()} /><button className="btn btn-ghost btn-sm" onClick={add}>+ Agregar</button></div>
      {saved && <div className="ok-msg" style={{ marginTop: 10 }}>{CH.check + " Guardado"}</div>}
      <div className="modal-foot"><button className="btn btn-outline" onClick={onClose}>Cancelar</button><button className="btn btn-ghost" onClick={handleSave}>{saving ? "Guardando..." : "Guardar"}</button></div>
    </div></div>
  );
};

/* DASHBOARD */
const Dashboard = ({ tasks, areas, weekOffset, setWeekOffset }) => {
  const targetMonday = useMemo(() => { const m = getMonday(); m.setDate(m.getDate() - (weekOffset * 7)); return m; }, [weekOffset]);
  const targetSunday = useMemo(() => getSunday(targetMonday), [targetMonday]);
  const fmtShort = (d) => { const dd = d.getDate().toString().padStart(2,"0"); const mm = (d.getMonth()+1).toString().padStart(2,"0"); return dd + "/" + mm; };

  const areaData = useMemo(() => {
    return areas.map((ar) => {
      const areaTasks = tasks.filter((t) => t.area === ar.nombre);
      const total = areaTasks.length;
      if (total === 0) return { nombre: ar.nombre, total: 0, completado: 0, en_proceso: 0, pendiente: 0, bloqueado: 0, cancelado: 0, avance: 0, allUpdated: true };
      const byE = {};
      ESTADOS.forEach((e) => { byE[e.id] = areaTasks.filter((t) => t.estado === e.id).length; });
      const avgAvance = Math.round(areaTasks.reduce((s, t) => s + (t.avance || 0), 0) / total);
      const allUpdated = areaTasks.every((t) => {
        if (!t.updated_at) return false;
        const upd = new Date(t.updated_at);
        return upd >= targetMonday && upd <= targetSunday;
      });
      return { nombre: ar.nombre, total, ...byE, avance: avgAvance, allUpdated };
    });
  }, [tasks, areas, targetMonday, targetSunday]);

  const globalStats = useMemo(() => {
    const total = tasks.length;
    const byE = {};
    ESTADOS.forEach((e) => { byE[e.id] = tasks.filter((t) => t.estado === e.id).length; });
    const avgAvance = total > 0 ? Math.round(tasks.reduce((s, t) => s + (t.avance || 0), 0) / total) : 0;
    return { total, ...byE, avance: avgAvance };
  }, [tasks]);

  return (
    <div>
      <div className="week-nav">
        <button className={"week-btn" + (weekOffset === 0 ? " active" : "")} onClick={() => setWeekOffset(0)}>Esta semana</button>
        <button className={"week-btn" + (weekOffset === 1 ? " active" : "")} onClick={() => setWeekOffset(1)}>Semana pasada</button>
        <button className={"week-btn" + (weekOffset === 2 ? " active" : "")} onClick={() => setWeekOffset(2)}>Hace 2 semanas</button>
        <button className={"week-btn" + (weekOffset === 4 ? " active" : "")} onClick={() => setWeekOffset(4)}>Mes pasado</button>
        <span className="week-label">{fmtShort(targetMonday) + " - " + fmtShort(targetSunday)}</span>
      </div>
      <div className="stats">
        <div className="stat"><div className="label">Total General</div><div className="value" style={{ color: "var(--black)" }}>{globalStats.total}</div><div className="sub">{globalStats.avance}% avance promedio</div></div>
        <div className="stat"><div className="label">En Proceso</div><div className="value" style={{ color: "var(--blue)" }}>{globalStats.en_proceso || 0}</div></div>
        <div className="stat"><div className="label">Completadas</div><div className="value" style={{ color: "var(--green)" }}>{globalStats.completado || 0}</div></div>
        <div className="stat"><div className="label">Pendientes</div><div className="value" style={{ color: "var(--yellow)" }}>{globalStats.pendiente || 0}</div></div>
        <div className="stat"><div className="label">Bloqueadas</div><div className="value" style={{ color: "var(--danger)" }}>{globalStats.bloqueado || 0}</div></div>
      </div>
      <h2 style={{ fontSize: 15, fontWeight: 700, marginBottom: 14, color: "var(--black)" }}>{"Resumen por " + CH.A + "rea"}</h2>
      <div className="dash-grid">
        {areaData.map((a) => (
          <div className="dash-card" key={a.nombre}>
            <div className="dash-card-head">
              <span className="dash-card-title">{a.nombre}</span>
              <span className="dash-check">{a.allUpdated && a.total > 0 ? CH.checkG : (a.total > 0 ? CH.warn : CH.dash)}</span>
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6 }}>
              <ProgressBar value={a.avance} size="lg" />
              <span style={{ fontFamily: "var(--mono)", fontSize: 13, fontWeight: 700 }}>{a.avance}%</span>
            </div>
            <div className="dash-stats">
              <span className="dash-stat">Total: <b>{a.total}</b></span>
              <span className="dash-stat" style={{ color: "var(--green)" }}>{"Comp: "}<b>{a.completado || 0}</b></span>
              <span className="dash-stat" style={{ color: "var(--blue)" }}>{"Proc: "}<b>{a.en_proceso || 0}</b></span>
              <span className="dash-stat" style={{ color: "var(--yellow)" }}>{"Pend: "}<b>{a.pendiente || 0}</b></span>
              {(a.bloqueado || 0) > 0 && <span className="dash-stat" style={{ color: "var(--danger)" }}>{"Bloq: "}<b>{a.bloqueado}</b></span>}
            </div>
          </div>
        ))}
      </div>
...
export default function App() {
  ...
  useEffect(() => { sessionCheckRef.current = setInterval(() => { if (user) { const s = getSession(); if (!s) setUser(null); else refreshSession(); } }, 60000); return () => clearInterval(sessionCheckRef.current); }, [user]);

  const handleLogin = (u) => { const s = setSession(u); setUser(s); };
  const handleLogout = () => { clearSession(); setUser(null); setPage("dashboard"); setTab("todas"); setFiltro("todos"); setFiltroPri("todos"); };

  const loadData = useCallback(async () => {
    const [areasRes, tasksRes] = await Promise.all([supabase.from("areas").select("id,nombre").order("id"), supabase.from("actividades").select("*").order("id")]);
  ...
  const saveTask = useCallback(async (task) => {
    ...
    const row = { area: task.area, actividad: task.actividad, responsable: task.responsable, fecha_inicio: task.fecha_inicio || null, fecha_limite: task.fecha_limite, estado: task.estado, prioridad: task.prioridad || "media", notas: task.notas || "", descripcion: task.descripcion || "", avance: task.avance || 0, visto_gerencia: isGer ? true : false };
  ...
  const deleteTask = useCallback(async (id) => { await supabase.from("actividades").delete().eq("id", id); setTasks((p) => p.filter((t) => t.id !== id)); setConfirmDelete(null); }, []);
  const markSeen = useCallback(async (task) => { if (user?.role === "gerente" && !task.visto_gerencia) { await supabase.from("actividades").update({ visto_gerencia: true }).eq("id", task.id); setTasks((p) => p.map((t) => t.id === task.id ? { ...t, visto_gerencia: true } : t)); } setDetailTask(task); }, [user]);

  const updAreas = useCallback(async (newAreas) => {
    const current = areas.map((a) => a.nombre); const updated = newAreas.map((a) => a.nombre);
    const removed = current.filter((n) => !updated.includes(n));
    for (const n of removed) { await supabase.from("areas").delete().eq("nombre", n); }
    for (const a of newAreas) { if (a.isNew) { await supabase.from("areas").insert({ nombre: a.nombre, password: a.password }); } else if (a.changed && a.password) { await supabase.from("areas").update({ password: a.password }).eq("nombre", a.nombre); } }
    await loadData();
  }, [areas, loadData]);
  ...
  const filtered = useMemo(() => {
    ...
    return r.sort((a, b) => { if (a.estado === "completado" && b.estado !== "completado") return 1; if (a.estado !== "completado" && b.estado === "completado") return -1; return (daysDiff(a.fecha_limite) ?? 999) - (daysDiff(b.fecha_limite) ?? 999); });
    ...
    <div className="app">
      ...
      <div className="topbar">
        <div className="topbar-left"><div className="topbar-brand">
          <CajaLogo size={50} /><span className="name">caja <b>cusco</b></span><span className="pipe" />
      ...
      <div className="main">
        <div className="nav-tabs">
          ...
          <button className={"nav-tab" + (page === "actividades" ? " active" : "")} onClick={() => setPage("actividades")}>Actividades</button>
        </div>
          ...
          <div className="table-wrap">
            <div className="table-bar">
              ...
              <div className="table-actions">
                <select className="filter-sel" value={filtro} onChange={(e) => setFiltro(e.target.value)}><option value="todos">Todos los estados</option>{ESTADOS.map((s) => <option key={s.id} value={s.id}>{s.label}</option>)}</select>
                <select className="filter-sel" value={filtroPri} onChange={(e) => setFiltroPri(e.target.value)}><option value="todos">Toda prioridad</option>{PRIORIDADES.map((p) => <option key={p.id} value={p.id}>{p.label}</option>)}</select>
                <button className="btn btn-ghost btn-sm" onClick={() => { setEditTask(null); setShowForm(true); }}>+ Nueva Actividad</button>
              </div>
            </div>
            {filtered.length === 0 ? (<div className="empty"><div className="icon">{CH.clip}</div><p>No hay actividades</p></div>) : (
              <div className="responsive"><table>
                <thead><tr>{isG && <th>{CH.A + "rea"}</th>}<th>Actividad</th><th>Responsable</th><th>Inicio</th><th>{"L" + CH.i + "mite"}</th><th>Estado</th><th>Avance</th><th>Notas</th><th></th></tr></thead>
                <tbody>
                  {filtered.map((t) => {
                    ...
                    return (
                      <tr key={t.id} onDoubleClick={() => markSeen(t)} style={isUnseen ? { background: "rgba(196,30,58,0.03)" } : undefined}>
                        ...
                        <td><select className="inline-sel" value={t.estado} onChange={(e) => updField(t.id, "estado", e.target.value)} onClick={(e) => e.stopPropagation()} style={{ background: est?.bg, color: est?.text, fontWeight: 600, borderColor: est?.bg }}>{ESTADOS.map((s) => <option key={s.id} value={s.id}>{s.label}</option>)}</select></td>
                        <td><div className="progress-cell"><ProgressBar value={t.avance} /><input className="progress-input" type="number" min="0" max="100" value={t.avance || 0} onChange={(e) => updField(t.id, "avance", Math.max(0, Math.min(100, parseInt(e.target.value) || 0)))} onClick={(e) => e.stopPropagation()} />%</div></td>
                        ...
                        <td><div style={{ display: "flex", gap: 4 }}>
                          ...
                          {isG && <button className="btn btn-danger-outline btn-sm" onClick={(e) => { e.stopPropagation(); setConfirmDelete(t); }}>{CH.trash}</button>}
                        </div></td>
                      </tr>
                    );
                  })}
